...
export function dashboard(req, res) {
  ...
  const billsByCategory = [];
    ContasFinanceiro.findAll()
      .then(bills => {
        const totalPaid = bills.filter(b => b.status === 'paid').reduce((sum, b) => sum + Number(b.value), 0);
        const totalOpen = bills.filter(b => b.status !== 'paid').reduce((sum, b) => sum + Number(b.value), 0);
        const billsByCategory = [];
        bills.forEach(b => {
          if (b.status === 'paid') {
